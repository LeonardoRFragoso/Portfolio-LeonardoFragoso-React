...
export default function Projects() {
  ...
  const topProjects = projects.filter(p => p.topForRecruiters);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div className="text-center mb-12">
            <div className="inline-flex items-center space-x-3 px-6 py-3 bg-gradient-to-r from-yellow-400/10 to-amber-500/10 border-2 border-yellow-400/30 rounded-full mb-4">
              <Award className="w-6 h-6 text-yellow-400" />
              <h3 className="text-2xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-amber-500">
                {t.projects.topForRecruiters}
              </h3>
            </div>
            <p className="text-white/70">{t.projects.topForRecruitersSubtitle}</p>
          </div>
            ...
            {topProjects.map((project, index) => {
              ...
              return (
                <motion.div
                  ...
                  className="group relative rounded-2xl overflow-hidden bg-gradient-to-br from-black/90 via-yellow-400/10 to-black/90 border-2 border-yellow-400/40 backdrop-blur-md shadow-2xl hover:shadow-yellow-400/40 transition-all duration-500"
                >
                  {/* Badge TOP */}
                  <div className="absolute top-4 left-4 z-20 px-3 py-1 bg-gradient-to-r from-yellow-400 to-amber-500 text-black text-xs font-bold rounded-full flex items-center space-x-1">
                    <Star className="w-3 h-3 fill-black" />
                    <span>TOP {index + 1}</span>
                  </div>

                  {/* Badge Avaliação Rápida */}
                  <div className="absolute top-4 right-4 z-20 flex items-center space-x-1 px-3 py-1 bg-black/70 backdrop-blur-sm rounded-full border border-white/20">
                  ...
                  <div className="p-6 space-y-3">
                    <h4 className="text-lg font-bold text-white group-hover:text-yellow-300 transition-colors line-clamp-1">
                      {project.title}
                    </h4>
                    ...
                    <div className="flex items-center space-x-1">
                      <span className="text-xs text-white/60 mr-2">{t.projects.complexity}:</span>
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-3 h-3 ${
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      {project.tags.slice(0, 3).map((tag, tagIndex) => (
                        <span
                          key={tagIndex}
                          className="px-2 py-1 rounded-full text-xs font-medium bg-yellow-400/10 text-yellow-300 border border-yellow-400/20"
                        >
                          {tag}
                        </span>
                      ))}
                      {project.tags.length > 3 && (
                        <span className="px-2 py-1 rounded-full text-xs font-medium bg-yellow-400/10 text-yellow-300 border border-yellow-400/20">
                          +{project.tags.length - 3}
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center space-x-3 pt-3">
                      {project.demo && (
                        <a
                          ...
                          className="flex-1 px-4 py-2 bg-gradient-to-r from-green-400 to-emerald-500 text-black text-sm font-bold rounded-lg hover:from-green-300 hover:to-emerald-400 transition-all flex items-center justify-center space-x-2"
                      ...
                      {project.demoLinks && project.demoLinks.length > 0 && (
                        ...
                        >
                          <ExternalLink className="w-4 h-4" />
                          <span>{t.projects.multipleApps}</span>
                        </button>
                      )}
                      {project.demoLinks && project.demoLinks.length > 0 && (
                        <button
                          onClick={() => openModal(project)}
                          className="p-2 bg-black/60 border border-yellow-400/20 hover:border-yellow-400/40 rounded-lg transition-colors"
                          aria-label={`${t.projects.viewGallery} - ${project.title}`}
                        >
                          <Eye className="w-4 h-4 text-white hover:text-yellow-400 transition-colors" />
                        </button>
                      )}
                      {project.github && (
                        <a
                        ...
                        >
                          <Github className="w-4 h-4 text-white hover:text-yellow-400 transition-colors" />
                        </a>
                      )}
                    </div>
                  </div>
